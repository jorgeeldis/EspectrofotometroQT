...
from PyQt5.QtWidgets import QWidget, QFileDialog, QAction, QLineEdit
...
class Ui_MainWindow(QWidget):
    def setupUi(self, MainWindow):  # holds the GUI
        ...
        self.actionSave_As = QAction("&Save File", self)
        ...
        self.actionAbsorption_at_Single_Wavelength.setObjectName(
            "actionAbsorption_at_Single_Wavelength"
        )
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionMeasure_Baseline.setText(
            _translate("MainWindow", "Measure Baseline")
        )
        self.actionAbsorption_at_Single_Wavelength.setText(
            _translate("MainWindow", "Absorption at Single Wavelength")
        )
    ...
    def saveFileDialog(
        self,
    ):  # opens SaveAs dialog and allows to save the already existing absorption file under whichever name the user chooses
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getSaveFileName(
            None, "Save As", "", "Text Files (*.txt)", options=options
        )  # selectedFilter='*.txt')
    ...
    def Baseline_meas(
        self,
    ):  # measure the baseline (the spectrum of the light source without the sample)
        progress = 0
        ...
        open(
            "baseline.txt", "w+"
        )  # opens the file baseline.txt (located in the same place as this script) and saves the measured values
    ...
    def Capture(self):
        ...
        for i in range(10, 225):
            ...
            if baseline2[i] <= 0:
                baseline2[i] = 1
            spectrum2[i] = (
                spectrum[i] + spectrum[i - 1] + spectrum[i + 1]
            ) / 3 - 980 * accum
            if spectrum2[i] <= 0:
                spectrum2[i] = 1
